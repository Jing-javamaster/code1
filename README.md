...
11月1日
static和final的区别❓
final是不能改；static是属于类，不属于对象

final：
1.修饰变量，表示不能改
2.修饰方法，表示这个方法不能被子类重写
3.修饰类，表示这个类不能被子类继承

//也就是对应的主体 有什么途径可以改变的 final就禁止那个途径

static：
1.修饰变量，所有对象共享这一份变量，不需要创建对象就能访问
2.修饰方法，表示一个不需要通过对象调用的方法，通过对象名.方法名❓❓❓就能调用
3.修饰内部类，表示这个内部类不依赖外部类实例。‼️很复杂（⚠️用 static 修饰的类都是内部类，不能是外部类，因为static的语义是 属于外部类，外部类已经是顶层，没有上一级）
{
普通内部类（非 static）依赖于外部类的实例存在。所以要调用它的方法，必须先有外部类对象，再通过这个对象创建内部类对象。
而我们说的static修饰的内部类，他的意义就在于 这个内部类里面的方法调用是不依赖于外部的实例的，也就是说我们可以直接通过（外部类名.内部类名）创建一个实例来调用内部类中的方法
}
‼️static 的本质作用是——让成员属于类本身，而不是某个对象。
‼️static 的本质作用是让成员属于类本身而非对象，使其在类加载时就存在，并被所有实例共享。
‼️静态内部类不能访问外部类的“实例成员”；只能访问外部类的“静态成员”。
❓1.被所有实例共享所以不能访问我理解，但为什么只能访问外部类的静态成员？
✅：因为静态内部类站在“类层级”的角度，它能看见类级别的东西（static），但看不见对象级别的东西（实例变量）。（666
）
所以内部类的应用原理是：当内部类被 static 修饰时，它就成为外部类的一个类级成员（属于类本身而非对象），因此可以直接通过“外部类名.内部类名”来创建或调用，而不需要外部类的实例。

static final：表示全局常量，static属于类表示全局，final表示不可修改的常量
...
修饰符					可访问范围					用途
public					所有类都能访问（无论在哪个包）		想给外界调用的接口、方法、构造函数
private					只能在当前类内部访问			封装内部实现，不让外部直接碰
protected				当前类 + 同包内的类 + 子类‼️	继承时常用
（默认，无修饰符）			仅同一个包内能访问				叫“包级可见”（package-private）
